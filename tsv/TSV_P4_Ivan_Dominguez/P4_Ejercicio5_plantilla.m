...
v1=[1/fs 0];
v2=[0 1/fs];
x=0:(1/fs):1; % conservamos la última muestra
y=0:(1/fs):1; % conservamos la última muestra
[X,Y]=meshgrid(x,y);
...
if (tipo==1)
    ...
    f_norm = fc/fs;
    H = (abs(Xc) <= f_norm) & (abs(Yc) <= f_norm); 

elseif (tipo==2)
    % filtro paso bajo ideal circular con frecuencia de corte fc
    param = sqrt(Xc.^2 + Yc.^2);
    H = double(param<=fc/fs); % usamos tipo double para que sea exacto
elseif (tipo==3)
...
C=sum(sum(filter_mask)); % de teoría
filter_mask= (1/C)*filter_mask; 

%% V. Medida error de diseño
% filtro truncado en espacio ("completo" => size(h))
h_t=zeros(size(h_desp));
h_t(cy-w : cy+w, cx-w : cx+w)= filter_mask;
h_t = (1/C)*h_t;

% Cálculo filtro truncado en frecuencia
H_T=fftshift(fft2(ifftshift(h_t)));
...
[ima_original, map]=imread('P4imas&code/edificio_bw_512.bmp');
