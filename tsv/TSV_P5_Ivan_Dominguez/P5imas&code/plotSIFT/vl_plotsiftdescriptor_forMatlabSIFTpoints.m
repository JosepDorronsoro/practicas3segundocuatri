function h=vl_plotsiftdescriptor_forMatlabSIFTpoints(features,points,varargin)
%% adjusting for MatlAB SIFTpoints
% Matlab features is pointsx128 single ... d is 128xpoints uint8
d=uint8(features');
% Matlab points is SIFTPoints
% F is a feature frame and has the format [X;Y;S;TH], where X,Y
%     is the (fractional) center of the frame, S is the scale and TH is
%     the orientation (in radians).

% Matlab o is x axis (right) and counterclockwise; vlt is y axis (down) and
% clockwise
points.Orientation=(3*pi/2)-(points.Orientation);

f=[ points.Location(:,1)'; points.Location(:,2)'; points.Scale'; points.Orientation'];

%function h=vl_plotsiftdescriptor_forMatlabSIFTpoints(d,f,varargin)
